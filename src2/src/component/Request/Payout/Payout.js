...
function Payout() {
  ...
  const [rValue, setRvalue] = useState(0.00)
  ...
  const fetchData = ()=>{
    try{
      axiosInstance.get(`Request/Request_Get_FundRequest`)
      .then(res=>{
let kk = []
        let arr = [];
        let obj = {}
 ...
 const requestInput = (e) => {
  ...
  <MyInput
   type="text"
   id={e.rowIndex}
   onChange={(k) => getRequestValue(k, e.rowIndex)}
value={requestValue[e.rowIndex]}
   placeholder="0.00"/>
     </>
   )
 }
  ...
  if (options.name === "payout") {
    if (options.summaryProcess === "start") {
      ...
      if (options.value.Type !== "Total") {
        options.totalValue += options.value.payOut;
      }
    }
  }
...
const saveDatafun = () => {
  ...
  axiosInstance.post("Request/Request_Post_FundRequest", details)
...
.then((res) =>{
  ...
  return (
    ...
    >
      ...
      <RadioWrapper>
          <span id="share">
          <input type="radio" id="share" name="share" defaultChecked value="share" />
...
<label htmlFor="fund">Fund</label>
        ...
        <div className={style.ammount}>Branch Request Amount : <span className="openingBalance"> {branchRequest} </span></div>
        <div className={style.bmsAmmount}>BMS Ammount : <span className="openingBalance">{resAmount}</span></div>
        <div className={style.buttonOk}>
          <MyButton variant="contained" onClick={saveDatafun}>Submit</MyButton>
        </div>
        <div className={style.buttonCancel}>
          <MyButton variant="">Cancel</MyButton>
        </div>
      </div>
    </Layout>
  );
